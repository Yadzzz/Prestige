...
public class ButtonHandler
{
    public static async Task HandleButtons(DiscordClient client, ComponentInteractionCreateEventArgs e)
    {
        ...
        // If the original message had an embed, keep it
        if (e.Message.Embeds.Count > 0)
        {
            builder.AddEmbed(e.Message.Embeds[0]);
        }

        // Update the original message: same embed, but buttons now disabled
        await e.Interaction.CreateResponseAsync(
            InteractionResponseType.UpdateMessage,
            builder
        );

        // Optional follow-up text so the user sees feedback
